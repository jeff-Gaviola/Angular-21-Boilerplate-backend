...
router.post('/forgot-password%0A', forgotPasswordSchema, forgotPassword);
router.post('/validate-reset-token', validateResetTokenSchema, validateResetToken);
router.post('/reset-password', resetPasswordSchema, resetPassword);
router.get('/', authorize(Role.Admin), getAll);
router.get('/all', authorize(Role.Admin), getAll);
router.get('/list', authorize(Role.Admin), getAll);
router.get('%0A', authorize(Role.Admin), getAll);
...
function verifyEmail(req: any, res: any, next: any) {
  ...
  const normalizedToken = decodeURIComponent(String(rawToken || '')).trim().replace(/^"|"$/g, '');

  accountService
    .verifyEmail({ token: normalizedToken })
...
function deleteAccount(req: any, res: any, next: any) {
  if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
    return res.status(401).json({ message: 'Unauthorized' });
  }
