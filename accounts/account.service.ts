...
async function register(params: any, origin: string) {
  ...
  if (existingAccount) {
    ...
    await sendVerificationEmail(existingAccount, origin);
...
function generateJwtToken(account: any) {
  return jwt.sign({ sub: account.id, id: account.id }, (config as any).secret, { expiresIn: '15m' });
}
...
async function sendVerificationEmail(account: any, origin: string) {
  let message;
  const tokenMessage = `<p>Verification token:</p><p><code>${account.verificationToken}</code></p>`;

  if (origin) {
    const verifyUrl = `${origin}/account/verify-email?token=${account.verificationToken}`;
    message = `<p>Please click the below link to verify your email address:</p><p><a href="${verifyUrl}">${verifyUrl}</a></p>${tokenMessage}`;
  } else {
    message = `<p>Please use the below token to verify your email address with the <code>/account/verify-email</code> api route:</p>${tokenMessage}`;
  }
